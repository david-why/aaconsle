...
prgmlist_disp:
    os_PutStrFull("prgm");
    os_PutStrFull(var_list[sel].name);
    os_SetCursorPos(2, 0);
    os_PutStrLine("vars=Lock mode=Hide X=Arc");
    os_NewLine();
    os_PutStrLine("enter=Execute del=Delete");
    while (1)
    {
        os_SetCursorPos(1, 0);
        os_PutStrLine("                 ");
        ...
        if (key == kClear)
            break;
        if (key == kEnter)
            program_runprgm(var_list + sel);
        // os_RunPrgm(program_list[sel].name, NULL, 0, runprgm_callback);
        // if (strcmp("AACONSLE", program_list[sel].name))
        // {
        if (key == kVars)
            program_togglelock(var_list + sel);
        if (key == kMode)
            program_togglehide(var_list + sel);
        if (key == kVarx)
            program_togglearc(var_list + sel);
        if (key == kDel)
        {
